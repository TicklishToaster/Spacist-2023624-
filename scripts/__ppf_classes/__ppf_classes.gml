...
function __ppfx_system() constructor {
	_pp_draw_enabled = true;
	_pp_render_enabled = true;
	_pp_time = 0;
	_pp_global_intensity = 1;
	_pp_stack_surface = array_create(12, -1);
	_pp_surf_index = 0;
	_pp_render_x = 0;
	_pp_render_y = 0;
	_pp_render_vw = 0;
	...
	//_pp_render_final_tex = -1;
	_pp_current_profile = noone;
	
	// misc: others
	_pp_sunshaft_small_surface = -1;
	_pp_sunshaft_downscale = 0;
	
	_pp_bloom_surface = array_create(16, -1);
	_pp_bloom_final_texture = -1;
	
	_pp_dof_surface = array_create(5, -1);
	_pp_dof_downscale = 0;
	
	_pp_kawase_blur_surface = array_create(16, -1);
	
	_pp_gaussian_blur_ping_surface = -1;
	_pp_gaussian_blur_pong_surface = -1;
	_pp_gaussian_blur_downscale = 0;
	
	static __cleanup = function() {
		__ppf_surface_delete_array(_pp_stack_surface);
		__ppf_surface_delete_array(_pp_bloom_surface);
		__ppf_surface_delete_array(_pp_dof_surface);
		__ppf_surface_delete_array(_pp_kawase_blur_surface);
	...
	_pp_cfg_default = {};
	__ppf_struct_copy(_pp_cfg, _pp_cfg_default);
}
#endregion
...
function pp_sine_wave(enabled, speedd=0.5, amplitude=[0.02, 0.02], frequency=[10, 10]) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		speedd : speedd,
...
function pp_displacemap(enabled, amount=0.1, zoom=1, speedd=0.1, angle=0, texture=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		amount : amount,
...
function pp_sunshafts(enabled, position=[0.5, 0.5], center_smoothness=0.3, threshold=0.5, intensity=3, dimmer=1.4, scattering=0.5, noise_enable=false, rays_tiling=1, rays_speed=0.03, downscale=0.8, dual_textures=false, world_tex=-1, sky_tex=-1, noise_tex=__PPF_ST.noise_perlin) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		position : position,
		...
		sky_tex : __ppf_is_undefined(world_tex) ? __PPF_ST.empty_texture : world_tex,
		noise_tex : noise_tex,
	};
}

function pp_bloom(enabled, iterations=5, threshold=0.4, intensity=4.5, colorr=c_white, dirt_enable=false, dirt_texture=-1, dirt_intensity=2.5, dirt_scale=1, reduce_banding=true, high_quality=true, debug=false) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
...
function pp_depth_of_field(enabled, radius=10, intensity=1, shaped=false, blades_aperture=6, blades_angle=0, use_zdepth=false, zdepth_tex=-1, focus_distance=0.2, focus_range=0.02, debug=false) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		radius : radius,
...
function pp_motion_blur(enabled, angle=0, radius=0, center=[0.5, 0.5], mask_power=0, mask_scale=1.2, mask_smoothness=1, overlay_texture=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		angle : angle,
...
function pp_texture_overlay(enabled, intensity=1, zoom=1, texture=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		intensity : intensity,
...
function pp_palette_swap(enabled, row=1, flip=false, texture=-1, pal_height=1, threshold=0, limit_colors=true) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		row : row,
...
function pp_chromaber(enabled, intensity=5, angle=35, only_outer=true, center_radius=0, blur_enable=false, prisma_lut_tex=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		angle : angle,
		...
		prisma_lut_tex : __ppf_is_undefined(prisma_lut_tex) ? __PPF_ST.default_prisma_lut : prisma_lut_tex,
	};
}

function pp_mist(enabled, intensity=0.5, scale=0.5, tiling=1, speedd=0.2, angle=0, contrast=0.8, powerr=1, remap=0.8, colorr=c_white, mix=0, mix_threshold=0, noise_tex=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		intensity : intensity,
...
function pp_speedlines(enabled, scale=0.1, tiling=5, speedd=2, rot_speed=1, contrast=0.5, powerr=1, remap=0.8, colorr=c_white, mask_power=5, mask_scale=1.2, mask_smoothness=1, noise_tex=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		scale : scale,
...
function pp_dithering(enabled, threshold=0, strength=0.3, mode=0, coord_absolute=false, bayer_texture=-1, bayer_size=8) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		threshold : threshold,
...
function pp_noise_grain(enabled, intensity=0.3, scale=1, mix=true, noise_tex=-1) constructor {
	effect_name = __ppf_effect_get_name(self);
	sets = {
		enabledd : enabled,
		intensity : intensity,
