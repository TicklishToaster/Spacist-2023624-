...
vec4 sample_box4(sampler2D tex, vec2 uv, float delta) {
	...
	vec4 d = texel_size.xyxy * vec2(-delta, delta).xxyy;
	vec4 col;
...
void main() {
	...
	vec3 bloom = sample_box4(bloom_tex, uv, 0.5).rgb * bloom_color * bloom_intensity * time_n_intensity.y;
	bloom = tonemap_jodie_reinhard(bloom);
	bloom = saturate3(bloom);
	if (bloom_dirt_enable > 0.5) {
		vec2 size = get_aspect_ratio(resolution.xy, vec2(bloom_dirt_scale));
