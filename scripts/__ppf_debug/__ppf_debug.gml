	...
	with(pp_index) {
		...
		switch(debug_type) {
			case PP_DEBUG_TYPE.STACK_SURFACES:
				...
				var j = 0, jsize = _pp_surf_index;
				repeat(jsize) {
					_xx = j mod 3;
					_yy = j div 3;
					_surf = _pp_stack_surface[j];
			...
			case PP_DEBUG_TYPE.CHANNELS_HISTOGRAM:
				...
				texture_set_stage(shader_get_sampler_index(__ppf_sh_histogram, "data_tex"), surface_get_texture(_pp_stack_surface[_pp_surf_index]));
