...
vec4 FXAA() {
	vec2 texel_size = vec2(1.0/resolution.xy);
	
	vec3 basecol = texture2D(gm_BaseTexture, v_vTexcoord).rgb;
	vec3 baseNW = texture2D(gm_BaseTexture, v_vTexcoord - texel_size).rgb;
	vec3 baseNE = texture2D(gm_BaseTexture, v_vTexcoord + vec2(texel_size.x, -texel_size.y)).rgb;
	vec3 baseSW = texture2D(gm_BaseTexture, v_vTexcoord + vec2(-texel_size.x, texel_size.y)).rgb;
	vec3 baseSE = texture2D(gm_BaseTexture, v_vTexcoord + texel_size).rgb;
	...
	dir = min(vec2(fxaa_strength), max(vec2(-fxaa_strength), dir * dirMin)) * texel_size;
