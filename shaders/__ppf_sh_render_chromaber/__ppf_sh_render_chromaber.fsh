...
// >> effect
vec4 chromaberration_fx(vec4 color, vec2 uv) {
	vec4 _col = color;
	vec2 texel = vec2(1.0/pos_res.zw);
	...
	if (chromaber_only_outer > 0.5) {
		...
		dist = vec2(texel * direction * uv2 * delta);
	} else {
		dist = vec2(texel * direction * chromaber_intensity);
	}
	dist *= time_n_intensity.y;
...
void main() {
	...
	col_final = chromaberration_fx(col_final, uv);
