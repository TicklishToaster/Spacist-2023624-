...
// >> effect
vec4 motion_blur_fx(vec4 color, vec2 uv) {
	vec4 _col = color;
	...
	vec2 dist = direction * mask * motion_blur_radius * 0.05 * time_n_intensity.y;
	...
	_col = blend_b(_col, texture2D(motion_blur_overlay_tex, uv));
	return _col;
}
...
void main() {
	...
	col_final = motion_blur_fx(col_final, uv);
